...
void settings_init(void) {
  ...
  if (persist_exists(KEY_BATTERY_DISPLAY)) {
    ...
    if (s_battery_display > BATTERY_WHEN_LOW) s_battery_display = BATTERY_ALWAYS;
  }
  if (persist_exists(KEY_COMPLICATION)) {
    s_complication = (ComplicationSlot)persist_read_int(KEY_COMPLICATION);
    if (s_complication > COMPLICATION_STEPS) s_complication = COMPLICATION_OFF;
  }
}
