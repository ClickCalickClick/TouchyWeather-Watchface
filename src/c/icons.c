...
// Face-only edit vs the app's icons.c: on 1-bit displays (diorite/flint)
// the hardcoded accent colors (ChromeYellow, VividCerulean) quantize to
// white and vanish on the light theme. The condition dispatchers below
// remap them to theme-aware fg/muted so icons stay visible in both themes.
// The upstream app never verified condition icons on B&W.
#if defined(PBL_BW)
...
void icon_draw_battery(GContext *ctx, GPoint c, int size, int pct,
                       bool charging, GColor color, GColor low_color) {
  if (pct < 0) pct = 0;
  if (pct > 100) pct = 100;
  int w = size;                 // body width
  int h = size * 5 / 8;         // body height
  if (h < 8) h = 8;
  int nub_w = 2;
  GColor fill = (pct <= 20) ? low_color : color;

  GRect body = GRect(c.x - w / 2, c.y - h / 2, w, h);
  set_stroke(ctx, color, 1);
  graphics_draw_round_rect(ctx, body, 2);
  // Terminal nub on the right.
  int nub_h = h / 2;
  graphics_context_set_fill_color(ctx, color);
  graphics_fill_rect(ctx, GRect(body.origin.x + w, c.y - nub_h / 2, nub_w, nub_h),
                     0, GCornersAll);
  // Proportional fill, inset 2px inside the outline.
  int inner_w = w - 4;
  int fill_w = (inner_w * pct) / 100;
  if (fill_w > 0) {
    graphics_context_set_fill_color(ctx, fill);
    graphics_fill_rect(ctx, GRect(body.origin.x + 2, body.origin.y + 2,
                                  fill_w, h - 4), 0, GCornersAll);
  }
  // Charging bolt overlaid in the center.
  if (charging) {
    graphics_context_set_stroke_color(ctx, (pct <= 20) ? color : low_color);
    graphics_context_set_stroke_width(ctx, 1);
    GPoint a = GPoint(c.x + 1, c.y - h / 2 + 2);
    GPoint b = GPoint(c.x - 2, c.y);
    GPoint d = GPoint(c.x + 2, c.y);
    GPoint e = GPoint(c.x - 1, c.y + h / 2 - 2);
    graphics_draw_line(ctx, a, b);
    graphics_draw_line(ctx, b, d);
    graphics_draw_line(ctx, d, e);
  }
}
